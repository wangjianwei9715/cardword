...
import { objKeySort } from "../tools/util";
export default class HttpRequest {
    constructor() {
        ...
        this.axiosInstance.interceptors.request.use((config) => {
            ...
            if (url.indexOf("me/goodTemplate/add") != -1) {
                let data = this.getStr(config.data) + '_addTemplate';
                config.headers['opSign'] = Md5.hashStr(data);
            }
            if (url.indexOf("me/goodTemplate/edit") != -1) {
                let data = this.getStr(config.data) + '_editTemplate';
                config.headers['opSign'] = Md5.hashStr(data);
            }
            if (url.indexOf("user/complain") != -1) {
                let data = this.getStr(config.data) + '_userComplain';
                config.headers['opSign'] = Md5.hashStr(data);
            }
            // 编辑商品
            if (url.indexOf("good/edit") != -1) {
                let data = this.getStr(config.data) + '_editGood';
                console.log(data);
                config.headers['opSign'] = Md5.hashStr(data);
            }
            // 支付订单
            if (url.indexOf("order/topay/") != -1) {
                let data = this.getStr(config.data) + '_payGoodOrder';
                config.headers['opSign'] = Md5.hashStr(data);
            }
            // 入驻保证金
            if (url.indexOf("topay/custom") != -1) {
                let data = this.getStr(config.data) + '_payCustom';
                config.headers['opSign'] = Md5.hashStr(data);
            }
            // 上传商家资质
            if (url.indexOf("user/sellerCertify/upload") != -1) {
                let data = this.getStr(config.data) + '_sellerCertifyUpload';
                config.headers['opSign'] = Md5.hashStr(data);
            }
            // 重新绑定手机号 第一步
            if (url.indexOf("user/reBindPhone/checkOldPhone") != -1) {
                let data = this.getStr(config.data) + '_checkOldPhone';
                config.headers['opSign'] = Md5.hashStr(data);
            }
            // 重新绑定手机号 第二步
            if (url.indexOf("user/reBindPhone/validCode1") != -1) {
                let data = this.getStr(config.data) + '_validCode1';
                config.headers['opSign'] = Md5.hashStr(data);
            }
            // 重新绑定手机号 第三步
            if (url.indexOf("user/reBindPhone/newPhone") != -1) {
                let data = this.getStr(config.data) + '_newPhone';
                config.headers['opSign'] = Md5.hashStr(data);
            }
            // 重新绑定手机号 第四步
            if (url.indexOf("user/reBindPhone/validCode2") != -1) {
                let data = this.getStr(config.data) + '_validCode2';
                config.headers['opSign'] = Md5.hashStr(data);
            }
            // 短信验证码
            ...
            if (url.indexOf("search") != -1) {
                if (config.url?.indexOf('scrollId') == -1) {
                    ...
                    console.log(data + '_searchSecret');
    ...
    Post(reqUrl, params, cb, errorCb) {
        let newParams = objKeySort(params);
        this.axiosInstance.post(reqUrl, newParams).then((response) => {
