			...
			<view class="HMSM">
				...
				<view class="HMSM-display">
					<view v-for="(shaft,index) in shaftList" :key="index" class="HMSM-shaft"
						:style="{transform: 'translate3d(0, '+translateY[index]+'%, 0)','transition-duration':(duration+(index*delay))+'ms'}"
						:class="{'roll_animation':(rollState=='start')}">
						<view v-for="(item,shaftIndex) in shaft" :key="shaftIndex">
							<image :src="decodeURIComponent(item.pic_url)" />
							{{item.name}}
						</view>
					</view>
				</view>
				...
				for (let i = 0, len = results.length; i < len; i++) {
					if(this.direction == 'up'){
						...
						for (let k = this.shaftList[i].length - 1; k >= 0; k--) {
							if (this.shaftList[i][k].id == results[i]) {
