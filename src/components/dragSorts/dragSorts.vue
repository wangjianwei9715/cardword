	...
	<view class="HM-drag-sort" :style="{'height': ListHeight+'px','background-color': listBackgroundColor}">
		...
		<view class="rowBox-shadow" id="shadowRowBox">
			<view class="hm-row-shadow move" id="shadowRow">
				<view class="modules">
					<!-- 内容  -->
					<slot name="listIndex" :data="shadowRow"></slot>
					<!-- 拖拽图标 -->
		...
		<!-- 拖拽列表 -->
		<view class="color scroll-view" :id="'scrollView_'+guid" :scroll-y="true"
			...
			<view class="list">
				<view v-for="(row,index) in dragList" :key="row.HMDrag_id" class="rowBox ani">
					...
					<view class="hm-row" :style="{'transform': 'translate3d(0,' + (row.HMDrag_sort-index)*100 + '%,-1px)'}" :data-sort="row.HMDrag_sort" :data-id="row.HMDrag_id" :id="row.HMDrag_id">
						<view class="modules">
							<!-- 内容  -->
							<slot name="listIndex" :data="row"></slot>
							<!-- 拖拽图标 -->
							<view v-if="row.disabled&&dragable" class="drag-content" >
								<view class="drag-icon" :style="{'height': rowHeight+'rpx'}">
									<text class="iconfont icon-drag disabled"></text>
								</view>
							</view>
							<view v-else-if="dragable" class="drag-content" :data-id="row.HMDrag_id" @touchstart="drag.touchstart"
			...
			pageScroll(e) {
				...
				if (e.command == "up" || e.command == "down") {
					...
					if (this.isAppH5) {
						...
						e.rowHeight = uni.upx2px(this.rowHeight);
...
<style lang="scss" scoped>
	...
	$Dark-border-color :rgba(174,174,178,0.3);
	...
	.iconfont {
		...
		&.icon-drag {
			width: 32rpx;
			height:28rpx;
			background: url(@/static/merchant/sort.png) no-repeat center / 100% 100%;
		}
