	...
	<view class="goodslist-content">
		<view v-show="goodsFalls" class="index-goods">
			<view class="goods-content goods-left">
				<view class="goods-item" v-for="item in goodsLeft" :key="item.target.id" @click="onClickJumpUrl(item.target.id)">
			...
			<view class="goods-content goods-right">
				<view class="goods-item" v-for="item in goodsRight" :key="item.target.id" @click="onClickJumpUrl(item.target.id)">
					<view class="goods-image-content"><image class="goods-image" :src="getGoodsImg(item.pic)" mode="widthFix"></image></view>
					<view class="goods-desc" v-html="item.title"></view>
					<view class="goods-price">￥{{item.price}}</view>
					<view class="goods-time">{{dateFormat(item.overAt)}} · {{item.bid}}</view>
		...
		<view v-if="!goodsFalls" class="index-content">
			<view class="goods-cross" v-for="item in goodsOldList" :key="item.target.id" @click="onClickJumpUrl(item.target.id)">
...
<script lang="ts">
	...
	export default class ClassName extends BaseComponent {
		...
		rightH = 0;
		@Watch('goodsFalls')
		onGoodsFallsChanged(val: any, oldVal: any){
			this.goodsFalls = val
		}
		@Watch('ispullDown')
		...
		getGoodTime(time:number){
			let hour=Math.floor(time/3600);
			let minute:any=Math.floor((time-3600*hour)/60);
			let second:any=Math.floor((time-3600*hour)%60);
			if(minute<10){
				minute = '0'+minute
			}
			if(second<10){
