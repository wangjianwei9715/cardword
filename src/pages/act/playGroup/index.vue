<template>
  <view class='playContent'>
    ...
    <view class="collectContainer" v-for="(item,index) in groupList" :key='index'>
      ...
      <view class="collect-card" :class="{noFourCollect:item.players.length<4}">
        <view class="cardItem" v-for="(man,manIndex) in item.players" :key='manIndex'>
          <muqian-lazyLoad class='cardItem-img' :class="{noneImg:man.haveNum==0}"
            @click='previewImage(item.players,manIndex,"pic")' :src="decodeURIComponent(man.pic)" alt=""
            mode="aspectFill" />
    ...
    <pedometerPop :show.sync='pedometerShow' :value.sync='selectItem.needExchangeNum'
      :maxValue.sync='selectItem.exchangeNum' @confirm="exchangeConfirm" />
      ...
      <template v-if="drawType==5">
        <view class="fiveCardContainer">
          <view class='fiveCardBlock' v-for='(item,index) in drawCard.list' :key='index'>
            <image class="fiveCardImage transitionAll" :class="{show:index<=drawCard.index}" :src="item.pic || ''"
              alt="" srcset="" @click='previewImage(drawCard.list,index)' mode='aspectFill' />
          </view>
        </view>
      </template>
      <template v-if="drawType==1">
        <image class="singleImage transitionAll" :class="{show:drawCard.index>=0}" :src="drawCard.list[0].pic || ''"
          alt="" srcset="" @click='previewImage(drawCard.list,0)' mode='aspectFill' />
      </template>
      <view class='transitionAll transitionDelay opacity0' :class="{opacity1:drawCard.index==drawCard.list.length-1}">
    ...
    <view style="z-index:-1;opacity:0;position:absolute;top:0;transform: scale(0);">
      <image v-for="(item,index) in drawCard.list" :key="index" :src="item.pic" mode="scaleToFill" @load='loadNum+=1'
        @error='loadNum+=1' />
    </view>
    <share :operationShow.sync='operationShow' @delyCallBack="shareCallBack" :shareData="shareData" />
    <u-toast ref="uToast"></u-toast>
    <view class="giveBlock" :class="{iosGiveBlock:platform=='ios'}" @click='pageJump("/pages/act/playGroup/give")'>
    </view>
    <view class="wechatModal" v-if="showWechat">
      <view class="modal">
        <view class="title" style="font-size: 42rpx;
        font-family: PingFang SC;
        font-weight: bold;
        margin-top: 50rpx;
        color: #333333;">提示</view>
        <view class="twoChart" style="font-size: 38rpx;
        font-family: PingFang SC;
        font-weight: 400;
        margin-top: 32rpx;
        color: #333333;">已复制口令</view>
        <view class="threeTips" style="font-size: 29rpx;
        font-family: PingFang SC;
        font-weight: 400;
        margin-top: 16rpx;
        color: #7C7C7C;">好友复制口令打开app即可助力</view>
        ...
        class="modalClose"
          src="../../../static/live/redPack_close.png"
          mode="scaleToFill"
        />
      </view>
    </view>
  ...
  export default class ClassName extends BaseNode {
    ...
    onLoad(query: any) {
      if (!app.token.accessToken) {
        uni.redirectTo({
          url: `/pages/login/login?redirect=/pages/act/playGroup/index?${query.helpCode ? "helpCode=" + query.helpCode : ""
            }`
        });
        return;
      }
    ...
    goWechat() {
      // #ifdef APP-PLUS
      plus.runtime.openURL('weixin://',  (res:any)=> {
        console.log(res);
    ...
    onClickDraw(type: number) {
      if (!app.token.accessToken) {
        uni.redirectTo({
          url: `/pages/login/login?redirect=/pages/act/playGroup/index?${this.helpCode ? "helpCode=" + this.helpCode : ""
            }`
        });
        return;
      }
    ...
    getHelpCode() {
      ...
      app.http.Post("activity/playerGroup/share/help", {}, (res: any) => {
        ...
        const PASSWROD = `亲，复制这段口令打开ka世界帮我助力集球员`
        const data = `${PASSWROD}${helpCode}`
        uni.setClipboardData({
          ...
          success: (res: any) => {
            ...
            //   title:'复制成功'
            // })
            uni.setStorageSync('mineGetHelpCode', helpCode)
    ...
    exchangeConfirm() {
      uni.showModal({
        title: "提示",
        content: `确认兑换${this.selectItem.needExchangeNum}份“${this.selectItem.name
          }”对应的奖励吗?`,
        success: (res: any) => {
