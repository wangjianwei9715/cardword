...
<template>
    <view class="content">
        ...
        <view class="dewuCoupon" v-for="(item, index) in list" :key="index"
            ...
            <view class="rightTips" v-if="item.canExchange">恭喜您，已兑换</view>
            <view class="rightTips" v-else>兑换后请在当天领取，次日作废</view>
            <view class="couponWrap" :class="{ couponWrap3: index == 1 }">
                <view class="coupon" :class="{ coupon3: index == 1 }">
                    <view class="couponFontContainer">
                        <view class="left">
                            <view class="name">卡牌评级/保护通用券</view>
                            <view class="time">领取后14天内有效</view>
                        </view>
        ...
        <view class="desc">1.每张得物卡牌评级/保护通用券可免费评级/保护一张卡片，<text
                style="color:#00feff;font-weight: bold;">≤135pt卡片可用。</text>每日限兑1次，次日作废</view>
        <view class="desc" style="margin-top: 20rpx;">2.用户在卡世界app兑换成功后，通过活动页面专属入口跳转到得物app领取评级券</view>
        <view class="desc" style="margin-top: 20rpx;">3.获得评级券的用户可登录得物app，通过 探索-鉴别服务-评级卡服务或直接在购买页搜索【卡牌评级】 按照操作流程进行评级。</view>
        <view class="imgContainer">
            <!-- <image mode="aspectFill" @click="prvImgRule(index)" v-for="(item, index) in picsRule" :key="index" :src="item">
            </image> -->
            <image mode="heightFix" @click="prvImgRule(0)" src="/static/act/dewu/rule1.png">
            </image>
            <image mode="heightFix" @click="prvImgRule(1)" src="/static/act/dewu/rule2.png">
            </image>
            <image mode="heightFix" @click="prvImgRule(2)" src="/static/act/dewu/rule3.png">
            </image>
        </view>
