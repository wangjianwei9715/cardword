	...
	<view class="content">
		...
		<view class="bottom" v-if="buckedList!=''">
			<view class="index" v-for="(item,index) in buckedList" :key="index" @click="onClickBucketId(item.bucketId)">
				<view class="left" >
					<view class="icon">
						<image class="icon-kf" :src="item.target.avatar?decodeURIComponent(item.target.avatar):'../../static/userinfo/kefu@2x.png'" mode="aspectFit"></image>
					</view>
					<view class="desc">
		...
		getBucketlist(){
			...
			app.http.Get('chat/bucketlist',params,(res:any)=>{
				...
				if(res.list){
					...
					console.log('buckedList=',res.list)
				}else{
