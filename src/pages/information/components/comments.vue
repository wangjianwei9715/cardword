<template>
	<view class="content">
		...
		<view v-for="(content, index) in contentArr" :key="index">
			...
			<video class="desc-video" :direction="0" :poster="`${videoArr[index]}?x-oss-process=video/snapshot,t_0,f_jpg`"
				v-if="videoArr[index] !== null" :src="videoArr[index]" controls></video>
		</view>
			...
			<view class="chat-box" v-for="(item, index) in commentsList" :key="index"
				...
				<view class="chat-item">
					...
					<view style="width:470rpx">
						...
						<text class="chat-msg" :selectable="true" :class="'chat-' + item.id"
							@click="onClickReply(item, item.id)">{{ item.content }}</text>
					</view>
				...
				<view class="chat-item son-item" v-for="(son, x) in item.lower" :key="x"
					...
					<view style="width:390rpx">
						...
						<view class="chat-msg" v-if="!son.isDelete" :class="'chat-' + son.id"
							@click="onClickReply(son, item.id)">
							<text class="chat-hf" :selectable="true" v-if="son.replyUserName != ''">回复<text
									class="hf-name">{{ son.replyUserName }}</text>:{{ son.content }}</text>
...
export default class ClassName extends BaseComponent {
	...
	touchAction(event: any, item: any) {
		console.log(item.id,this.touchId);
	...
	pickUpActionSheet(item:any) {
        ...
        plus.nativeUI.actionSheet({
            ...
            app.http.Post("comment/report/"+item.id,{tp:1,reason_tp:value},(res:any)=>{
                uni.showToast({
                    title:"举报成功",
                    icon:"none"
                })
