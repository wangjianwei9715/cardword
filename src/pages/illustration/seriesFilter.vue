<template>
	<view class="side-content">
		<view class="side-left">
			<view class="side-tab" :class="{'current':current.id==item.id}" v-for="(item,index) in showTab" :key="index" @click="onClickTab(item.id)">{{item.content}}{{item.id<=4?`(${item.num})`:""}}</view>
		</view>
			...
			<view class="right-box">
				<scroll-view :scroll-top="scrollTop" scroll-y="true" class="scroll-Y"  :scroll-with-animation="true" @scroll="onScroll">
					<view v-for="(item,index) in showTab" :key="index" :id="item.id" class="scroll-view-item">
						<view class="right-title">{{item.content}}</view>
						<view class="input-box" v-if="item.id<=4">
							<view class="search-icon"></view>
							<input v-if="item.name=='cardSet'" class="input-operate" v-model="cardSetParams.q" placeholder="搜索卡种中英文"/>
							<input v-else-if="item.name=='player'" class="input-operate" v-model="playerParams.q" placeholder="搜索球员中英文"/>
							<input v-else-if="item.name=='team'" class="input-operate" v-model="teamParams.q" placeholder="搜索球队中英文"/>
							<input v-else-if="item.name=='seq'" class="input-operate" v-model="seqParams.q" placeholder="搜索限编号"/>
							<view class="input-close" @click="onClickClearQ(index)"></view>
						</view>
						<view class="right-list" v-if="item.id==1">
							<view class="cardset-item u-line-2" :class="{'current-card':selectHasItem(card)}" v-for="(card,cindex) in item.list" :key="cindex" @click="onClickSelectItem(card)">{{card.content}}</view>
							<u-loadmore :status="item.status" line @loadmore="loadmore(item)"/>
						</view>

						<view class="right-list" v-if="[2,3,4].includes(item.id)">
							<view class="player-item u-line-1" :class="{'current-card':selectHasItem(card)}" v-for="(card,cindex) in item.list" :key="cindex" @click="onClickSelectItem(card)">{{card.content}}</view>
							<u-loadmore :status="item.status" line @loadmore="loadmore(item)"/>
						</view>
						...
						<view class="right-list" v-if="[7].includes(item.id)">
							<view class="num-item">
								<input class="num-input" v-model="item.numberFrom" placeholder="最低编号" type="number">
							</view>-
							<view class="num-item">
								<input class="num-input" v-model="item.numberEnd" placeholder="最高编号" type="number">
							</view>
						</view>
					</view>
				</scroll-view>
		...
		<view class="side-bottom">
			<scroll-view class="bottom-scroll" :scroll-x="true">
				<view class="bottom-scroll-box"  v-for="(item,index) in selectList" :key="index" @click="selectList.splice(index,1)">
					<view class="bottom-scroll-item">{{item.content}}<view class="icon-close"></view></view>
				</view>
			</scroll-view>
...
<script lang="ts">
	...
	export default class ClassName extends BaseNode {
		seriesCode = "";
		sideTab:any = [
			{id:1,name:'cardSet',content:'卡种',num:0,list:[{ "nameId": 1, "content": 'Craftsmen Press Proof Black' },{ "nameId": 5, "content": 'Craftsmen Press Proof BlackCraftsmen Press Proof Black' }],show:true,status:"loadmore"},
			{id:2,name:'player',content:'球员',num:0,list:[{ "nameId": 2, "content": '大萨达' }],show:true,status:"loadmore"},
			{id:3,name:'team',content:'球队',num:0,list:[{ "nameId": 3, "content": '而我却二群无' }],show:true,status:"loadmore"},
			{id:4,name:'seq',content:'限编',num:0,list:[{ "nameId": 4, "content": '心辞职信' }],show:true,status:"loadmore"},
			{id:5,name:'rc',content:'新秀',show:true,tp:null,tab:['新秀','非新秀']},
			{id:6,name:'signature',content:'签字',show:true,tp:null,tab:['签字','非签字']},
			{id:7,name:'number',content:'编号范围',show:true,numberFrom:"",numberEnd:""},
		];
		current = {
			id:1,
			execution:false
		...
		initEvent(){
			app.http.Get(`dataApi/cardIllustration/series/${this.seriesCode}/search/options/overview`,{},(res:any)=>{
				this.sideTab.forEach((x:any)=>{
					if(x.id<=4){
						x.num = res.data[x.name];
					}else if([5,6].includes(x.id)){
						x.show = res.data[x.name];
					}
				})
		...
		getListOfIndex(index:number){
			...
			app.http.Get(`cardIllustration/series/${this.seriesCode}/search/options/${item.url}`,params,(res:any)=>{
				if(res.list){
		...
		onClickSelectItem(item:any){
			...
			!repeat && this.selectList.push(item)
		}
		...
		onClickFilterTab(item:any,tab:string){
			...
			if(findContent == null){
				this.selectList.push({nameId:-1,content:tab});
			}else{
				this.selectList.forEach((x,index)=>{
					if(x.content == item.tp){
						this.selectList.splice(index,1)
					}
				})
				item.tp!=tab && this.selectList.push({nameId:-1,content:tab});
			}
