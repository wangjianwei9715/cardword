		...
		<view class='follow uni-flex' v-for="(item,index) in followList" :key='index' @click='toDetail(item)'>
			<template v-if="item.follow">
				...
				<view class="follow-action">
					<followButton :follow='item.follow||true' :followID='item.id' @handleSuccess='followSuccess($event,item,index)'></followButton>
				</view>
			</template>
		</view>
...
export default class ClassName extends BaseNode {
  ...
  toDetail(item: any) {
    uni.navigateTo({
      url:
        "/pages/userinfo/merchant_shopsV2?id=" +
        item.id +
        "&chaneListKey=followList"
    });
  }
