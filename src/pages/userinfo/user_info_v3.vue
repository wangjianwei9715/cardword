<template>
    <view class="content">
        <view class="orther-setting">
            <view class="setting" v-for="item in settingTab" :key="item.id" @click="onClickEdit(item.id)">
                <!-- 头像或名称 -->
                <view v-if="item.id == 1">
                ...
                <view v-else class="right" @click="onClickC(item.id)">
                    <view class='sign'>{{ item.bol != undefined ? (item.bol ? "公开" : "私密") : item.desc }}</view>
                    <view class="icon-right"></view>
                </view>
            </view>
        </view>
...
export default class ClassName extends BaseNode {
    ...
    onClickEdit(id: any) {
        ...
        if (id == 8) {
            uni.showModal({
                title: "提示",
                content: `是否${this.settingTab.pub_card.bol ? "隐藏" : '公开'}中卡`,
                success: (res: any) => {
                    if (res.confirm) {
                        app.http.Post(`cardCircle/${this.settingTab.pub_card.bol ? "un/" : ""}pub/hit/card`, {}, () => {
                            this.settingTab.pub_card.bol = !this.settingTab.pub_card.bol
                        })
                    }
                }
            })
    ...
    getData() {
        app.http.Get('cardCircle/me/home', {}, (res: any) => {
            // console.log(res);
            const data = res.data
            for (const key in this.settingTab) {
                if (Object.prototype.hasOwnProperty.call(data, key)) {
                    ...
                    } else if (key == 'pub_card') {
                        this.settingTab[key].bol = data[key]
                    } else {
