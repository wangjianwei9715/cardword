<template>
	<view class="content">
		<view class="header-banner">
			...
			<view class="tab-header">
				...
				<view class="header-right uni-flex">
					<img src="../../static/userinfo/v2/back.png" alt="" @click="onClickBack">
					<img src="../../static/userinfo/v2/search.png" style="width:35rpx" alt="">
				</view>
		...
		<view class="tabsMenu uni-flex">
			<view class="menuItem" :class="{selectItem:index==goodTabV2.index}" @click="tabsChange(item,index)" v-for="(item,index) in goodTabV2.list" :key='index'>
				{{item.name}}
...
export default class ClassName extends BaseNode {
  ...
  onLoad(query: any) {
    ...
    if (query.chaneListKey) this.chaneListKey = query.chaneListKey;
  }
  ...
  onClickBack() {
    ...
    if (this.chaneListKey) {
      for (let i = 0; i < prevPage[this.chaneListKey].length; i++) {
        if (this.detail.id == prevPage[this.chaneListKey][i].id) {
          prevPage[this.chaneListKey][i].follow = this.detail.followed;
          prevPage[this.chaneListKey][i].fans = this.detail.fans;
          break;
        }
      }
	  console.log(prevPage[this.chaneListKey])
    }
    uni.navigateBack({
      delta: 1
    });
