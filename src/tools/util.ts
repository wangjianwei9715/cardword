...
export function getUrlData(urlStr: string) {
	const urlObj: {[key: string]: string | undefined} = {};
	const regex = /[?&]+([^=&]+)=([^&]*)/gi;
	urlStr.replace(regex, function(match, key, value) {
		urlObj[key] = value === '' ? undefined : value;
		return '';
	});
	return urlObj;
}
