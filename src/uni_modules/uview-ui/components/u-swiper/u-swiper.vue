<template>
	<view class="u-swiper" :style="{
		...
		<swiper v-else class="u-swiper__wrapper" :style="{
			...
			:nextMargin="$u.addUnit(nextMargin)" :acceleration="acceleration" :displayMultipleItems="displayMultipleItems"
			...
			<swiper-item class="u-swiper__wrapper__item" v-for="(item, index) in list" :key="index">
				<view class="u-swiper__wrapper__item__wrapper" :style="[itemStyle(index)]">
					...
					<video class="u-swiper__wrapper__item__wrapper__video" v-if="getItemType(item) === 'video'"
						...
						:poster="getPoster(item)" :title="showTitle && $u.test.object(item) && item.title ? item.title : ''"
						:style="{
							height: $u.addUnit(height)
		...
		<view class="u-swiper__indicator" :style="[$u.addStyle(indicatorStyle)]">
			<slot name="indicator">
				<u-swiper-indicator v-if="!loading && indicator && !showTitle" :indicatorActiveColor="indicatorActiveColor"
	...
	methods: {
		getItemType(item) {
			if (typeof item === 'string') {
				if (this.getSource(item).indexOf("x-oss-process=video/snapshot") >= 0) return "image"
